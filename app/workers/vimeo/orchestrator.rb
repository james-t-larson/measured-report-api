  ...
  class Orchestrator
    ...
    def perform(work_remaining: nil)
      ...
      if (transcript = Transcript.in_progress.first.presence)
        Rails.logger.info("[Vimeo::Orchestrator] Starting Transcript processor")
        Vimeo::TranscriptProcessor.perform_in(jittered_delay, transcript.id)
        return
      elsif (video = Video.in_progress.first.presence)
        Rails.logger.info("[Vimeo::Orchestrator] Starting video processor")
        Vimeo::VideoProcessor.perform_in(jittered_delay, video.id)
        return
      end

      if work_remaining?
        Rails.logger.info("[Vimeo::Orchestrator] Starting Content Orchestator")
        Vimeo::Orchestrator.new.perform
      end
    end
    ...
    def future_job_already_scheduled?
      Rails.logger.info("[Vimeo::Orchestrator] Checking for future jobs")
